...
def add_dmat_evals_to_hdf5(fptr: Union[h5py.File, h5py.Group],
                           time: numpy.ndarray, eigenvalues: numpy.ndarray):
    group = fptr.create_group("eigenvalues")
    group.create_dataset("time", time.shape, time.dtype)[:] = time
    group.create_dataset("eigenvalues", eigenvalues.shape,
                         eigenvalues.dtype)[:, :] = eigenvalues
...
def add_dmat_evecs_grid_to_hdf5(fptr: Union[h5py.File,
    ...
    group.create_dataset("real", evecs.shape,
                         evecs.real.dtype)[:, :, :] = evecs.real
    group.create_dataset("imag", evecs.shape,
...
def add_dmat_evecs_spf_to_hdf5(fptr: Union[h5py.File, h5py.Group],
                               time: numpy.array, evecs: numpy.ndarray):
...
def add_dmat_spfrep_to_hdf5(fptr: Union[h5py.File, h5py.Group],
                            time: numpy.ndarray, dmat: numpy.ndarray):
    group = fptr.create_group("dmat_spfrep")
    group.create_dataset("time", time.shape, time.dtype)[:] = time
    group.create_dataset("real", dmat.shape,
                         dmat.real.dtype)[:, :, :] = dmat.real
    group.create_dataset("imag", dmat.shape,
                         dmat.imag.dtype)[:, :, :] = dmat.imag
