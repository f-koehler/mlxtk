...
def read_momentum_distribution_ascii(
    ...
    data = pandas.read_csv(path,
                           sep=r"\s+",
                           names=["times", "momenta", "distribution"])
    ...
    distribution = numpy.reshape(data["distribution"].to_numpy(),
...
def read_momentum_distribution_hdf5(
    ...
    with h5py.File(path, "r") as fptr:
        return fptr[interior_path]["times"][:], fptr[interior_path][
            "momenta"][:], fptr[interior_path]["distribution"][:, :]
...
def add_momentum_distribution_to_hdf5(fptr: Union[h5py.File, h5py.Group],
                                      times: numpy.ndarray,
                                      momenta: numpy.ndarray,
                                      distribution: numpy.ndarray):
    grp = fptr.create_group("momentum_distribution")
    grp.create_dataset("times", shape=times.shape,
                       dtype=times.dtype)[:] = times
    grp.create_dataset("momenta", shape=momenta.shape,
                       dtype=momenta.dtype)[:] = momenta
    grp.create_dataset("distribution",
                       shape=distribution.shape,
                       dtype=distribution.dtype)[:, :] = distribution
